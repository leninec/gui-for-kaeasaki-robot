...
int UdpClient::getPointCoordint(int nP, QString timeoutText, float *x,float *y,float *z,float *o,float *a,float *t)
{
    ...
    if (this->SendCommand(Data,"coordpoint","Ошибка получения координаты точки",2)  )
    {}
    else
    {
        {
...
int UdpClient::UploadShift()
{
    QByteArray Data;
    Data.append("8;");
    if (this->SendCommand(Data,"shift","Ошибка получения смещения",2)  )
    {
        return 1;
    }
    {
        ...
        if (i ==0 )
        {
            emit error("Потерял данные в векторе сообщений получение смещения");
            return 1;
        }
        Data = this->vPpriemMessage[i-1];
...
int UdpClient::HereShift() // функция, берущая текущую координату и сдвигающая на нее массив точек
{
    ...
    if (i ==0 )
    {
        emit error("Потерял данные в векторе сообщений");
        return 1;
    }
...
int UdpClient::ResetError()
{
    ...
    Data.clear();
    Data.append("56;");
    if (this->SendCommand(Data,"error","Ошибка квитирования робота",2)) return 1;
    {
        QMutexLocker locker(&vPpriemMessage_mutex);
    ...
    if (i == 0 )
    {
        emit error("Потерял данные в векторе сообщений квитирование");
        return 1;
    }
    {
        QMutexLocker locker(&vPpriemMessage_mutex);
...
int UdpClient::WaitingMoveFinish()
{
    ...
    {
        this->pFazus->Stop_fazus();
        emit error("Потерял данные в векторе сообщений ожидание завершения движения 3 точка");
        return 1;
    }
    QByteArray Data;
    {
...
int UdpClient::GetErrorRobot()
{
    ...
    QByteArray data;
    data.append("55;");
    if (this->SendCommand(data,"error","Ошибка получения статуса робота",2))return 1;
    ...
    if (i == 0 )
    {
        emit error("Потерял данные в векторе сообщений получения статуса робота");
        return 1;
    }
    {
        QMutexLocker locker(&vPpriemMessage_mutex);
    ...
    {
        {
            ...
            if (i == 0 )
            {
                emit error("Потерял данные в векторе сообщений получение статуса мотора");
                return 1;
            }
            while (i > 0)   //24.08/16 в SendComand теперь перебираем весь вектор принятых сообщений поэтому првоеряем и здесь где именно лежит ответ
            {
    ...
    {
        {
            ...
            if (i ==0 )
            {
                emit error("Потерял данные в векторе сообщений получение статуса цифровых входов");
                return 1;
            }
            Data = this->vPpriemMessage[i-1];
            QString str(Data);
