    ...
    ui(new Ui::Widget)
{
    ...
    // ui->label_18->setText(" После калибровки робот расположен на метке ноль градусов."
    //                        "Необоходимо опустить его в начальную точку, учитывая высоту выбранной траектории сканирвания");

    //  ui->label_6->setWordWrap(true);
    //  ui->label_6->setText(" Для калибровки небоходимо вывести датчики по обе стороны втулки в районе метки 0 градусов так,"
    //                       "чтобы на временной развертке видеть эхо сигнал от передней грани");
    ...
    ui->pushButtonCalibration->setToolTip("Для запуска процедуры калибровки расположите ики так, чтобы видеть эхо сигнал от детали ");
...
void Widget::ReadAnswer()
{
    ...
    if(str != 0)
    {
        ...
        {
            ui->plainTextEdit->appendPlainText("["+data.toString("HH:mm")+"]"  +" Калибровка не завершена. Проверьте местоположение втулки и датчиков и повторите процедуру. " );
            this->bMassivButton[bcalibrovka]= true;
            this->bMassivButton[bnastrUZK]= true;
            this->bMassivButton[bgetTraect]= true;
            this->bMassivButton[bgetUzk]= true;
        ...
        if (str.contains("calibrationfinish",Qt::CaseInsensitive))
        {
            ...
            //нужно подложить фазусу имя настройки
            /*
            // до этого сначала выбирали имя настрорйки а потом псоле калибровкии ее открывали
        ...
        if (str.contains("контрольокончен",Qt::CaseInsensitive))
        {
            ...
            msgBox1.setInformativeText("Необходим повторный контроль этой втулки? (Без повторной калибровки, и без изменения траектории сканирования");
            msgBox1.setIcon(QMessageBox::Question);
...
void Widget::HereShift()
{
    QMessageBox msgBox;
    msgBox.setWindowTitle(" Управление роботом RS10");
    msgBox.setText("Датчики расположены в начальной точке(под деталью, на метке 0 градусов)?");
    //  msgBox.setInformativeText(" Процедура контроля была прервана. Сохранить записанные данные?");
    msgBox.setIcon(QMessageBox::Question);
...
void Widget::Calibration()
{
    ...
    //  msgBox.setInformativeText(" Процедура контроля была прервана. Сохранить записанные данные?");
    msgBox.setIcon(QMessageBox::Question);
    QPushButton *yes = msgBox.addButton(tr("Да, можно начинать калибровку"), QMessageBox::ActionRole);
    QPushButton *no = msgBox.addButton(tr("Нет"), QMessageBox::ActionRole);
    msgBox.exec();
    if(msgBox.clickedButton()== yes)
    {
        ui->plainTextEdit->appendPlainText("["+data.toString("HH:mm")+"]"  +" Началась процедура калибровки ");
        this->bMassivButton[bcalibrovka]= false;
        this->bMassivButton[bnastrUZK]= false;
